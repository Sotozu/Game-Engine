...
namespace FieaGameEngine
{
	...
	class StaticMesh final : public Action
	{
		RTTI_DECLARATIONS(StaticMesh, Action);
	...
	public:
		...
		/// </returns>
		inline gsl::owner<StaticMesh*> Clone() const override;
		...
		StaticRegisterMethod(StaticMesh, Action);
	...
	protected:
		...
		StaticMesh(std::size_t typeId);

		//ADDED 6/21/2022
		StaticMesh(Game& game, std::size_t typeId);
	...
	private:
		...
		void CreateIndexBuffer(gsl::not_null<ID3D11Device*> device, const Mesh& mesh, gsl::not_null<ID3D11Buffer**> indexBuffer);
		...
		std::shared_ptr<Model> _staticModel;
		Mesh* _staticMesh;
	};

	ConcreteFactory(StaticMesh, Action);
}
