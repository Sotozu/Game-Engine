...
namespace FieaGameEngine
{
	...
	AnimationPlayer::AnimationPlayer(Game& game, shared_ptr<ModelResource> model, bool interpolationEnabled) :
		GameComponent(game),
	...
	const shared_ptr<ModelResource>& AnimationPlayer::GetModel() const
	{
		return mModel;
	}
	...
	void AnimationPlayer::Update(const GameState& gameState)
	{
		if (mIsPlayingClip)
		{
			assert(mCurrentClip != nullptr);

			mCurrentTime += gameState._gTime.ElapsedGameTimeSeconds().count() * mCurrentClip->TicksPerSecond();
