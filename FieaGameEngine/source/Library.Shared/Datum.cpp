...
#pragma region Initializer List Template Specializations
	...
	template<>
	Datum::Datum(std::initializer_list<glm::vec4> list) : _capacity(list.size()), _type(DatumTypes::Vector), isExternal(0)
	{
...
#pragma endregion
	...
	void Datum::PushBackFromString(const std::string& str) //Should this function be able to call malloc and allocate space if an empty datum calls it?
	{
		...
		case ('v'):
			if (_type != DatumTypes::Vector) { throw std::runtime_error("Datum type and string token type not compatible."); }
			glm::vec4 myVec;
			sscanf_s(str.c_str(), "vec4(%f, %f, %f, %f)", &myVec[0], &myVec[1], &myVec[2], &myVec[3]);
			PushBack(myVec);
			break;
...
#pragma region Find Overloads
	...
	{
		if (_type != DatumTypes::Vector) { throw std::runtime_error("Wrong Datum type call."); }
