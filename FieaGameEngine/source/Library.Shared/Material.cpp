...
namespace FieaGameEngine
{
	...
	Material::Material(Game& game, D3D_PRIMITIVE_TOPOLOGY topology) : Attributed(Material::TypeIdClass()),
		mGame(&game),
		mTopology(topology)
	{
	}
...
#pragma region Attributed Required Methods
	...
	//std::shared_ptr<Shader>& Material::GetVSShader()
	//{
	//	const auto&[found, data] = GetDataForShaderStage(ShaderStages::VS);

	//}
	//std::shared_ptr<Shader>& Material::GetPSShader()
	//{

	//}


	Material::Material(D3D_PRIMITIVE_TOPOLOGY topology) : Attributed(Material::TypeIdClass()),
		mTopology{topology},
		mGame{ nullptr },
		_pixelShaderFilePath{""},
		_vertexShaderFilePath{""}
	{

	}

	Material::Material(std::size_t typeId, D3D_PRIMITIVE_TOPOLOGY topology) : Attributed(typeId),
		mTopology{ topology },
		mGame{nullptr},
		_pixelShaderFilePath{ "" },
		_vertexShaderFilePath{ "" }
	...
	bool Material::Equals(const RTTI* rhs) const
	{
		const Material* material = rhs->As<Material>();
	...
	void Material::Update(const GameState& gameState)
	{
		UNREFERENCED_PARAMETER(gameState);
	}


	const Vector<Signature> Material::Signatures()
	{
		return Vector<Signature>
		{
			{ "_vertexShaderFilePath", DatumTypes::String, 1, offsetof(Material, _vertexShaderFilePath) },
			{ "_pixelShaderFilePath", DatumTypes::String, 1, offsetof(Material, _pixelShaderFilePath) },
		};

	}


	Material::Material(Game& game, std::size_t typeId, D3D_PRIMITIVE_TOPOLOGY topology) : Attributed(typeId),
		mTopology{ topology },
		mGame{ &game },
		_pixelShaderFilePath{ "" },
		_vertexShaderFilePath{ "" }
