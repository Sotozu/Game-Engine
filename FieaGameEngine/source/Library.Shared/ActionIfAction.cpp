...
namespace FieaGameEngine
{
	...
	bool ActionIfAction::CheckTrueFalseSettings()
	{
		...
		{
			_target = new Datum;
			int32_t& myInt = SearchUp(_targetKey)->Get<int32_t>();
			_target->SetStorage(&myInt, 1);
		}
		if (_true == nullptr)
		{
	...
	void ActionIfAction::Update(const GameTime& gameTime)
	{
		if (!CheckTrueFalseSettings()) { return; }

		if (_target->Get<int32_t>() > 0)
		{
			_true->Update(gameTime);
		}
		else
		{
			_false->Update(gameTime);
		}
