...
{
	BoneAnimation::BoneAnimation(ModelResource& model, InputStreamHelper& streamHelper) :
		mModel(&model)
	{
		Load(streamHelper);
	}

	BoneAnimation::BoneAnimation(ModelResource& model, const BoneAnimationData& boneAnimationData) :
		mModel(&model), mBone(model.Bones().at(boneAnimationData.BoneIndex)), mKeyframes(boneAnimationData.Keyframes)
	{
	}

	BoneAnimation::BoneAnimation(ModelResource& model, BoneAnimationData&& boneAnimationData) :
	...
	bool BoneAnimation::operator==(const BoneAnimation& rhs) const
	{
		return &mBone == &rhs.mBone &&
			mKeyframes == rhs.mKeyframes;
	}
