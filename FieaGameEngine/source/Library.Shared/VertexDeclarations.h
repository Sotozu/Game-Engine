...
#include <d3d11.h>
#include <gsl\gsl>
#include <GameException.h> //NOTE: Added here. There is ThrowIfFail call that has no context... I need to know how Paul go that to work...
#include <utility.h>
#include <array>
namespace FieaGameEngine
{
	...
	{
	private:
		inline static const std::vector<D3D11_INPUT_ELEMENT_DESC> _InputElements
		{
	...
	public:
		...
		inline static const std::vector<D3D11_INPUT_ELEMENT_DESC> InputElements { _InputElements };
		...
		inline static const std::vector<D3D11_INPUT_ELEMENT_DESC>& GetDescription()
		{
	...
	void VertexDeclaration<T>::CreateVertexBuffer(gsl::not_null<ID3D11Device*> device, const gsl::span<const T>& vertices, gsl::not_null<ID3D11Buffer**> vertexBuffer)
	{
		...
		ThrowIfFailed(device->CreateBuffer(&vertexBufferDesc, &vertexSubResourceData, vertexBuffer), "ID3D11Device::CreateBuffer() failed."); //This is from GameException class
	}
