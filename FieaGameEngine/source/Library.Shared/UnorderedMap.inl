...
template<typename TKey, typename TData>
typename UnorderedMap<TKey, TData>::size_type UnorderedMap<TKey, TData>::Size()
{
	return _size;
}
...
typename UnorderedMap<TKey, TData>::it_bool_pair UnorderedMap<TKey, TData>::Find(const TKey& key)
{
	...
	chain_iterator chain_it = _buckets[bucket].begin();

	Iterator It = end();
	bool isFound = false;
	while (chain_it != _buckets[bucket].end())
	{
		if (_equalityFunctor((*chain_it).first, key))
		{
			It.SetInnerIterator(bucket, chain_it);
			isFound = true;
			break;
		}
		++chain_it;
...
typename UnorderedMap<TKey, TData>::const_it_bool_pair UnorderedMap<TKey, TData>::Find(const TKey& key) const
{
	...
	size_type bucket = BucketPlacement(key);
	const_chain_iterator chain_it = _buckets[bucket].begin();
...
{
	Iterator It(*this,0);
	chain_iterator it = _buckets[0].begin();
	size_type bucket_index = 0;
	
	for (bucket_index; bucket_index < _buckets.Size(); ++bucket_index)
	{
		it = _buckets[bucket_index].begin();
		if (it != _buckets[bucket_index].end())
		{
			break;
		}
	}
	It.SetInnerIterator(bucket_index, it);
	return It;
}
...
typename UnorderedMap<TKey, TData>::ConstIterator UnorderedMap<TKey, TData>::begin() const
{
	...
	const_chain_iterator it = _buckets[0].begin();
	size_type bucket_index = 0;
