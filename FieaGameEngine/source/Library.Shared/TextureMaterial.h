...
#pragma region REQUIRED METHODS TO SUPPORT RTTI AND ATTRIBUTED DESIGN
	public:
		...
		/// </returns>
		gsl::owner<TextureMaterial*> Clone() const override;
		...
		StaticRegisterMethod(TextureMaterial, Material);
	...
	protected:
		...
		TextureMaterial(std::size_t typeId);
...
#pragma endregion
	...
	public:
		TextureMaterial(Game& game, std::shared_ptr<Texture2D> colormap);
		...
		TextureMaterial& operator=(TextureMaterial&&) = default;
	...
	ConcreteFactory(TextureMaterial, Scope);
