...
#pragma region Get Template Specilizations
	...
	{
		if (_type != DatumTypes::Vector) { throw std::runtime_error("Cannot use Get method on Datum with type not set."); }
		if (index >= _size) { throw std::runtime_error("Cannot access empty datum or go beyond current size of datum."); }
...
#pragma region Front Template Specializations
	...
	{
		if (_type != DatumTypes::Vector) { throw std::runtime_error("Template function is not compatible with Datum type."); }
		if (_size == 0) { throw std::runtime_error("Cannot access empty Datum."); }
...
#pragma region Back Template Specializations Const
	...
	{
		if (_type != DatumTypes::Vector) { throw std::runtime_error("Cannot access unassigned Datum."); }
...
#pragma region Set Overloads
	...
	{
		if (_type != DatumTypes::Vector) { throw std::runtime_error("Cannot call this overloaded set function on Datum type Datum::Vector."); }
		if (index >= _size) { throw std::runtime_error("Cannot access beyond current size of datum."); }
		_data.v[index] = value;
	}
...
#pragma region OO Is Equal
	...
	{
		if (_type != DatumTypes::Vector || _size != 1 || _data.v[0] != value) { return false; }
		return true;
	}
