...
namespace UnitTestLibraryDesktop
{
	...
	public:
		...
		TEST_METHOD(Action_ActionIncrement)
		{
			...
			myActionIncrement->Update(myGameTime);
		...
		TEST_METHOD(Action_ActionCreateAction)
		{
			...
			myRootGameObject->Update(myGameTime); //Creating an ActionIntegerIncrement Action. Aslo avatar runs it's own update method which increments its _hitPoints by 1
			Assert::AreEqual(static_cast<std::size_t>(1), actionsDatum->Size());
			gState.FinalizeUpdate(); //Adopting that ActionIntegerIncrement Action to it's target.
		...
		TEST_METHOD(Action_ActionDestroyAction)
		{
			...
			myRootGameObject->Update(myGameTime); //Queuing the deletion of an ActionIntegerIncrement Action.
			gState.FinalizeUpdate();
			Assert::AreEqual(static_cast<std::size_t>(1), actionsDatum->Size());
			...
			myRootGameObject->Update(myGameTime); //At this time the ActionIntegerIncrement Action has been removed.
			gState.FinalizeUpdate();
			Assert::AreEqual(static_cast<std::size_t>(1), actionsDatum->Size());
