...
namespace UnitTestLibraryDesktop
{
	...
	void AvatarFoo::Update(const GameTime& gameTime)
	{
		_hitPoints++;
	...
	FieaGameEngine::Vector<FieaGameEngine::Signature> AvatarFoo::Signatures()
	{
		return Vector<Signature>
		{
			...
			{"_isTakingDamage"s, DatumTypes::Integer, 1, offsetof(AvatarFoo, _isTakingDamage)}
	...
	bool AvatarFoo::Equals(const RTTI* rhs) const
	{
		...
		return _name == myAvatarFoo->_name &&
			_transform._position == myAvatarFoo->_transform._position &&
			_transform._rotation == myAvatarFoo->_transform._rotation &&
			_transform._scale == myAvatarFoo->_transform._scale &&
